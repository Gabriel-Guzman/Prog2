...
int ** buildMatrix(int size) {
	...
	return matrix; //Creating a matrix as a double pointer so that we can pass it easily
}
...
int ** buildMagicSquare(int size) {
	...
	while(true) {
		if (matrix[currentX][currentY] != -1) { //After choosing the next slot, if
			break;								//the current slot is full, end
		}
...
//The next two functions mirror the magic square
